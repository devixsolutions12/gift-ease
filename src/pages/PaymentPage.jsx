...
// QR code will be imported dynamically to avoid build errors if file doesn't exist
...
const PaymentPage = () => {
  ...
  // Payment settings state - loaded from local storage
  const [paymentSettings, setPaymentSettings] = useState({
    upiId: 'mgas8604@oksbi',
  ...
  // Load payment settings from local storage on component mount
  useEffect(() => {
    ...
    const handleStorageChange = (e) => {
      if (e.key === 'giftEasePaymentSettings') {
        try {
          ...
          console.log('PaymentPage: Storage event received, updating settings', updatedSettings);
      ...
      try {
        ...
        if (savedSettings) {
          ...
          setPaymentSettings(prevSettings => {
            ...
            if (prevStr !== newStr) {
              console.log('PaymentPage: Polling detected settings change, updating', parsedSettings);
    ...
    console.log('PaymentPage: Started polling for updates every 2 seconds');
    
    // Cleanup event listener and interval
    return () => {
      console.log('PaymentPage: Cleaning up event listener and interval');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const orderData = {
        ...
        amount: parseInt(currentProduct.price.replace('₹', '')),
      ...
      <header className="header">
        <div className="container">
          ...
          <nav className="nav">
            ...
            <button onClick={() => navigate('/track-order')} className="nav-button">
              Track Order
            </button>
                  ...
                  <ol className="steps-list">
                    ...
                    <li>
                      <div className="step-content">
                        ...
                        <span className="step-text">Enter the exact amount: <strong>{currentProduct.price}</strong></span>
                      </div>
                    </li>
                    <li>
                      <div className="step-content">
